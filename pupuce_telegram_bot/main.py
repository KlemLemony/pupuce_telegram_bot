...
async def start_cmd_handler(bot: Bot):
    await bot.send_message(-1001610055755, f'У тебя все хорошо? Да или нет?')
    await dp.current_state(chat=-1001610055755, user=335261939).set_state(Form.is_everything_ok)

@dp.message_handler(text='да', state=Form.is_everything_ok)
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='ну и чудненько')
...
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='если честно, мне все равно. Я просто спросил')
    await asyncio.sleep(2)
    await message.answer("Клятва на пальчиках в силе? Мы еще охана?")
    await Form.next()
...
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='я тебя не понимаю')

@dp.message_handler(text='да', state=Form.remember_oath)
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='япикаееееей, мазафака. Тогда до встречи на следующей неделе! Счастья, здоровья тебе')
    await state.finish()

@dp.message_handler(text='нет', state=Form.remember_oath)
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='Пидор')
    await state.finish()

@dp.message_handler(state=Form.remember_oath)
async def start_cmd(message: types.Message, state: FSMContext):
    await bot.send_message(chat_id = -1001610055755, text='чего блять? Ты пожешь написать да или нет?')
    
@dp.message_handler(text=['пупусь','pupuce', 'пу', 'Пупусь','Pupuce', 'Пу'])
async def pupuce(message: types.Message):
    await message.answer("че надо? Иди нахуй")
    await asyncio.sleep(1)
    await bot.send_sticker(chat_id = -1001610055755, sticker='CAACAgIAAxkBAAEIKHFkEwiKU-xDCnNIoNFwUq37X88AAVgAAjYJAAJ5XOIJQ8ZQFdq6zjcvBA')
